...
const parse = async () => {
  ...
  try {
    ...
    let result = { resturant: 'Toka Kök', week: 0, days: [] };
    let weekday = '';
    let parseVeckans = false;
    let veckans = []
    let widget = $('div div div div div div');
    widget.children().each((i, elem) => {
      ...
      } else if (text.indexOf('Veckans') === 0) {
        parseVeckans = true;
      } else if (parseVeckans) {
        $(elem).find('span').each((i, pElem) => {
